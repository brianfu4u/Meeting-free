// GENERATED_AGENT_V11_MIRROR source=src/lib/phase4/triggerCore.js
...
export function buildUnifiedRunRequest({ clinicId, businessDate, slot, policyVersion, triggerType, artifacts }) {
  ...
  const watermarkRows = (Array.isArray(artifacts) ? artifacts : [])
    .map((item) => ({ seq: Number(item?.ingestion_seq), ingestedAt: item?.ingested_at || item?.captured_at || item?.created_date || null }))
  ...
  return {
    ...
    request: {
      ...
      cutoff_ingested_at: watermarkRows.map((item) => item.ingestedAt)
        .filter((value) => typeof value === "string" && value).sort().at(-1) || null,
