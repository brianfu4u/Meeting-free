...
if (!Array.isArray(input.scenarios) || input.scenarios.length < 1 || input.scenarios.length > 200) throw new Error("scenario_count_invalid");
...
for (const scenario of input.scenarios) {
  ...
  else if (level === "approximate") approximate.push({ scenario_id: scenario.scenario_id, scenario_type: scenario.scenario_type, oracle: scenario.oracle, reason });
...
if (supported.length < 1 || supported.length > 200) throw new Error(`supported_scenario_count_invalid_${supported.length}`);
