...
async function execute(scenario, actor) {
  ...
  try {
    ...
    for (const spec of scenario.fixture.workflows || []) {
      ...
      const wf=await create("Workflow",{clinic_id:clinic,workflow_family:clean(spec.workflow_family),subject_type:clean(spec.subject?.type),subject_fingerprint:{...fp,...(spec.device_serial?{device_serial:String(spec.device_serial)}:{}),name:clean(fp.name,`SUBJECT-${spec.key}`)},status,temporal_anchors:[nowIso],started_at:new Date(now.getTime()-600000).toISOString(),last_event_at:nowIso,open_loops:Array.isArray(spec.open_loops)?spec.open_loops:[],session_id:`session-${crypto.randomUUID()}`});
