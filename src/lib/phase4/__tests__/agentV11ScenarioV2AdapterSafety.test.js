...
describe("scenario v2 fixture adapter safety",()=>{
  it("executes exactly supported fixtures and separates non-production evidence",()=>{
    ...
    expect(build).toContain("supported.length !== 12"); expect(runner).toContain("approximate_report"); expect(runner).toContain("unsupported_skipped"); expect(runner).toContain("excluded_from_eligible_rate:true");
  });
