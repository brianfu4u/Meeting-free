/** Agent v1.1 authoritative attachment and Undo List projection. */
...
export function buildAttachmentLinkDescriptor({
  ...
  if (!["agent_autonomous", "manager_manual"].includes(decisionSource)) {
    throw new Error("decision_source_invalid");
  }
...
/**
 * Undo resolution is a projection of an already-persisted authoritative link.
 * Callers cannot resolve by passing only an artifact/workflow pair.
 */
