...
export default function PanelDrawer({ panel, onClose }) {
  ...
  const hourlyData = panel.detail?.hourlyFlow?.map((v, i) => ({
    时间: `${8 + i}:00`,
    人数: v,
  }));

  const revenueData = panel.detail?.dailyRevenue?.map((v, i) => ({
    天: `D${i + 1}`,
    营收: v,
  }));
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ borderBottom: "1px solid #1E3352", background: "#0D1B2A" }}
        >
            ...
            >
              <span
                ...
                style={{
                  ...
                  animation: panel.status === "red" ? "pulseRed 1.5s ease-in-out infinite" : "none",
                }}
              />
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Core metrics */}
          <div className="p-5">
            <div className="text-xs font-semibold mb-3" style={{ color: "#64748B", letterSpacing: "0.08em" }}>
              核心指标
            </div>
            <div className="grid grid-cols-2 gap-3">
              {panel.metrics.map((m, i) => (
                ...
                >
                  <div className="text-xs mb-1" style={{ color: "#64748B", fontSize: "10px" }}>
                    {m.label}
                  </div>
                  <div className="flex items-baseline gap-1">
                    <span className="font-bold" style={{ color: "#00C7D9", fontSize: "22px", lineHeight: "1" }}>
                      {m.value}
                    </span>
                    {m.unit && (
              ...
              >
                {panel.detail.subMetrics.map((sm, i) => (
                  <div
                    ...
                    style={{
                      borderBottom: i < panel.detail.subMetrics.length - 1 ? "1px solid #1A2D42" : "none",
          ...
          {hourlyData && (
            <div className="px-5 pb-5">
              ...
              <div
                ...
                style={{ background: "#1E293B", border: "1px solid #2D3F55", height: "140px" }}
              >
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={hourlyData}>
                    <XAxis dataKey="时间" tick={{ fill: "#475569", fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: "#475569", fontSize: 10 }} axisLine={false} tickLine={false} />
                    <Tooltip
                      contentStyle={{ background: "#0D1B2A", border: "1px solid #1E3352", borderRadius: "8px", fontSize: "11px", color: "#F1F5F9" }}
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={revenueData}>
                    <XAxis dataKey="天" tick={{ fill: "#475569", fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: "#475569", fontSize: 10 }} axisLine={false} tickLine={false} />
                    <Tooltip
                      contentStyle={{ background: "#0D1B2A", border: "1px solid #1E3352", borderRadius: "8px", fontSize: "11px", color: "#F1F5F9" }}
                      labelStyle={{ color: "#94A3B8" }}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{ background: "rgba(0,199,217,0.12)", color: "#00C7D9", border: "1px solid rgba(0,199,217,0.2)" }}
                        >
                          {s.id}
                        </div>
                        <div>
                          ...
                          <div className="text-xs" style={{ color: "#64748B", fontSize: "10px" }}>
                            {s.area}
                          </div>
                        </div>
                      </div>
                      <div
                        ...
                        style={{ background: sc.bg, color: sc.text, border: `1px solid ${sc.border}`, fontSize: "10px" }}
                      >
                        {s.status}
                      </div>
                    </div>
          ...
          {/* Staff involved (non-staff panel) */}
              ...
              <div className="flex flex-wrap gap-2">
                {panel.detail.staffInvolved.map((name, i) => (
                  <span
                    ...
                    style={{ background: "rgba(0,199,217,0.1)", color: "#00C7D9", border: "1px solid rgba(0,199,217,0.2)" }}
                  >
                    {name}
                  </span>
                ))}
